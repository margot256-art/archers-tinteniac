...
export default function StatsCoach() {
  ...
  const archerOptions = useMemo(() => {
    const names = [...new Set(
      seances
        .filter(s => s.date && (filterSaison === "Toutes" || getSaison(s.date) === filterSaison))
        .map(s => s.archer)
        .filter(Boolean)
    )].sort();
    return ["Tous", ...names];
  ...
  const rows = useMemo(() => {
    const src = seances.filter(s => {
      ...
      return okSaison && okArcher && okDist;
    });
  ...
  }, [seances, filterSaison, filterArcher, filterDist]);
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        ...
        <div style={s.filters}>
          <FilterSelect label="Saison"   value={filterSaison} options={saisons}        onChange={v => { setFilterSaison(v); setFilterArcher("Tous"); }} />
          ...
          <div style={s.tableWrap}>
            <table style={s.table}>
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={14} style={s.empty}>Aucune séance trouvée.</td>
                  </tr>
                ) : (
                  rows.map((row, i) => {
